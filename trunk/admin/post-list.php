		...
		<div id="page">
            ...
            <form action="<?php $options->adminUrl('post-list.php'); ?>">
				...
				<select name="status">
                    ...
                    <option value="all" <?php Typecho_Request::callParameter('status', 'all', 'selected="true"'); ?>>
                        <?php _e('所有文章 (%d)', Typecho_API::factory('*Widget_Count', "from=table.contents&count=cid&type[]=post&type[]=draft")->num); ?>
                    </option>
					<option value="allPost" <?php Typecho_Request::callParameter('status', 'allPost', 'selected="true"'); ?>>
                        <?php _e('所有已发布的文章 (%d)', Typecho_API::factory('*Widget_Count', "from=table.contents&count=cid&type=post")->num); ?>
                    </option>
					<option value="allDraft" <?php Typecho_Request::callParameter('status', 'allDraft', 'selected="true"'); ?>>
                        <?php _e('所有草稿 (%d)', Typecho_API::factory('*Widget_Count', "from=table.contents&count=cid&type=draft")->num); ?>
                    </option>
                    <?php endif; ?>
					<option value="my" <?php Typecho_Request::callParameter('status', 'my', 'selected="true"'); ?>>
					...
					<option value="myPost" <?php Typecho_Request::callParameter('status', 'myPost', 'selected="true"'); ?>>
                        <?php _e('我的已发布文章 (%d)', Typecho_API::factory('*Widget_Count', "from=table.contents&count=cid&author={$access->uid}&type=post")->num); ?>
                    </option>
					<option value="myDraft" <?php Typecho_Request::callParameter('status', 'myDraft', 'selected="true"'); ?>>
